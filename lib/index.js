var xmlparse,           // xml parser
    ...
    strings = {};       // we have to store some string constants as Titanium
                        // expects tag names to be case sensitive and jsdom
...
if (!isTi) {
   ...
   strings.speed = "speedkmhr";
   strings.stoptag = "stoptag";
   strings.dirtag = "dirtag";
   strings.routetag = "routetag";
   strings.stoptitle = "stoptitle";
   strings.routetitle = "routetitle";
   strings.dirTitleNoPred = "dirtitlebecausenopredictions";
   strings.lastTime = "lasttime";
   strings.secsSinceReport = "secssincereport";
} else {
   geohash = require('/vendor/nextbusjs/lib/geohash');
...
function client () {
      ...
      query("predictionsForMultiStops",
         ...
         function (err, response) {
            ...
            try {
               ...
               if (data.length === 0) {
                  Ti.API.debug(Ti.XML.serializeToString(response));
                  throw new Error('response is invalid');
               }

               for (i = 0; i < data.length; i++) {
                  stop = data.item(i).getAttribute(strings.stoptitle);
                  ...
                  for (j = 0; j < item.length; j++) {
                     if (direction !== 'null') {
                        ...
                        // ignore it
                        if (direction !== item.item(j).getAttribute(strings.dirtag)) {
                           continue;
                        }
                     }
      ...
      query("predictionsForMultiStops",
         ...
         try {
            ...
            for (i = 0; i < data.length; i++) {
               ...
               route = data.item(i).getAttribute(strings.routetitle);
               ...
               if (currDirectionNodes.length !== 0) {
                  ...
                  currDirection = data.item(i)
                                  .getAttribute(strings.dirTitleNoPred);
               }
               ret.push({
   ...
   */
   function closestStops (lat, lon, num, accuracy) {
      ...
      var loc = geohash.encode(lat, lon), nearest, stops;
      if (agencyData.active) {
         stops = agencyData.active.stops;
      } else {
         stops = agencyData.stopsByTitle;
      }
   ...
   */
    ...
    * Guesses which routes are currently active by checking a couple stops on
    * each route to see if there are times available.  Then, assumes that
    * every stop on each active route is active, yielding a list of active
    * routes and active stops for this agency.
    *
    * Parameters:
    *    accuracy    - *Number* of stops to check for each route.  Default 4
   ...
   function guessActive (accuracy, callback) {
      if (!isAgencyCached) {
      ...
      // build a query string containing queries for 4 stops on each route

      // loop over all routes
      for (route in agencyData.routes) {
         if (agencyData.routes.hasOwnProperty(route)) {
            // choose 4 stops randomly
            stops = [];
            len = agencyData.routes[route].stops.length;

            // add to query url using this route and those four stops
            for (i = 0; i < accuracy - 1; i++) {
               str += "&stops=" + route + "|null|" + 
                  agencyData.routes[route].stops[rand(len - 2)];
            }

            // also add the last 2 stops.  When a line is shutting down,
            // only the final stops will have predictions
            str += "&stops=" + route + "|null|" +
               agencyData.routes[route].stops[len - 1];
            str += "&stops=" + route + "|null|" +
               agencyData.routes[route].stops[len - 2];
         }
      }

      // run the query for multistops on the query string we built
      query("predictionsForMultiStops", str, function (err, response) {
         ...
         try {
            ...
            for (i = 0; i < data.length; i++) {
               route = data.item(i).getAttribute(strings.routetag);
               item = data.item(i).getElementsByTagName('prediction');
      ...
      query('vehicleLocations', str, function (err, response) {
         ...
         try {
            ...
            if (isTi) {
               ...
               lastTime = response.getElementsByTagName(strings.lastTime);
            } else {
               vehicles = response.document.getElementsByTagName("vehicle");
               lastTime = response.document.getElementsByTagName(strings.lastTime);
            }
            ...
            for (i = 0; i < vehicles.length; i++) {
               ...
               route = vehicle.getAttribute(strings.routetag);
               result[route] = result[route] || [];
               result[route].push({
                  id:      vehicle.getAttribute('id'),
                  dirtag:  vehicle.getAttribute(strings.dirtag),
                  ...
                  since:   vehicle.getAttribute(strings.secsSinceReport),
                  speed:   vehicle.getAttribute(strings.speed)
               });
            }
