...
if (!isTi) {
   ...
   request = function (url, callback) {
      ...
      xhr.onerror = function (e) {
         ...
         callback({name: "error"}, response, null);
      };
      xhr.open("GET", url);
      xhr.send();
   };
...
/*
   ...
   few seconds.  Every time a query is run, the query string is cached into the
   object.
...
function client () {
   ...
   */
   ...
   function routePredict (route, direction, cb) {
               ...
               for (i = 0; i < data.length; i++) {
                  ...
                  for (j = 0; j < item.length; j++) {
                     ...
                     ret[currIndex].predictions.push(
                        item.item(j).getAttribute('minutes')
                     );
                  }
   ...
   */
   ...
   function stopPredict (stop, direction, cb) { 
