...
if (!isTi) {
   ...
   geohash = require('models/nextbus/lib/geohash');
...
function client () {
      ...
      query("predictionsForMultiStops",
         ...
         function (err, response) {
            ...
            if (data.length === 0) {
               ...
               cb({name: 'Error', message: 'Response is invalid'}, null);
               return;
            }
   ...
   */
   function stopPredict (stop, direction, cb) { 
      ...
      if (agencyData.stops[stop] !== undefined) {
         ...
         cb({name: "nostop", message: "stop not found"}, null);
         return;
      }
      ...
      query("predictionsForMultiStops",
         ...
         if (data.length === 0) {
            Ti.API.debug(Ti.XML.serializeToString(response));
            cb({name: 'RangeError', message: 'Response is invalid'}, null);
            return;
         }
   ...
   /*
      ...
      Finds closest stops to a particular lat and lon
   ...
   */
   function closestStops (lat, lon, num, accuracy) {
      ...
      var loc = geohash.encode(lat, lon), nearest;
      nearest = geohash.nearest(loc, agencyData.stopsByTitle, num, accuracy);
      return nearest;
   }
